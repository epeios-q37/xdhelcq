...
var contentAttributeName = "data-xdh-content";
...
function fetchEventHandlersAndWidgets( id )
{
	...
	while ( cont ) {
		if ( node.nodeType == Node.ELEMENT_NODE ) {
			if ( node.hasAttribute( onEventAttributeName ) )
				eventsDigests += "(" + getOrGenerateId( node ) + "|" + node.nodeName + "|((" + node.getAttribute( onEventAttributeName ) + ")))|";

			if ( node.hasAttribute( onEventsAttributeName ) )
				eventsDigests += "(" + getOrGenerateId( node ) + "|" + node.nodeName + "|(" + node.getAttribute( onEventsAttributeName ) + "))|";
	...
	return "(" + eventsDigests + ")(" + widgetDigests + ")";
} 
...
// Returns list of elements which request content.
function fetchContents( id ) {
    ...
    while ( cont ) {
        if ( node.nodeType == Node.ELEMENT_NODE) {
            if (node.hasAttribute( contentAttributeName ) ) {
                digests += "(" + getOrGenerateId( node ) + "|" + node.getAttribute( contentAttributeName ) + ")|";
            }
        }
...
function instantiateWidgets( ids, types, parametersSets )
{
	...
	while ( i-- ) {
		script += instantiateWidget( ids, types, parametersSets[i] );
	}
